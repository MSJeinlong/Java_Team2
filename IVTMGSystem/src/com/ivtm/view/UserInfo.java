...
import com.demo2.UserUpdateDialog;
...
public class UserInfo extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "alter":
			...
			// 显示修改对话框
			// 匿名类
			new UserUpdateDialog(null, "修改信息", true, model, rowNum);
			// 更新数据模型
			model = new UserModel();
			// model.query("");
			this.query("");
		...
		// 删除
		case "delete":
			int[] rowNum1 = this.jtb.getSelectedRows();
			if (rowNum1 == null) {
				// 用户没选择任何一行时提示
				JOptionPane.showMessageDialog(this, "请选择一行", "警告！", JOptionPane.WARNING_MESSAGE);
				return;
				...
				if (n == 0) { // n == 0，表示用户点击了"是"；
					Model.update(null, rowNum1);// 执行删除操作
					// 弹出提示信息
	...
	public Model newModel() {
		Model m = MyTools.newTclass(model.getClass());
		return model;
